...
1. 관리자가 정산단위(RECURRING/CAMPAIGN)을 작성 → 즉시 배포.
2. 직원이 출장모드 시작 → TRIP 정산단위 자동 생성.
3. 직원이 영수증 촬영 → 원본 보존 + 크롭 + OCR.
4. 직원이 정산단위·비목을 확정 (자동추천 참고, 최종 선택은 사용자).
5. (Optional) 부가세 확인/수정, 적격증빙 경고 확인.
6. 직원이 "정산하기" → 영수증-카드내역 거래 매칭 (기존 로직).
7. 전표 자동생성 (정산단위 규칙 반영) → 직원 검토 → 상신.
8. 관리자 콘솔에 전표·결재선·정산단위 사용량이 즉시 반영.

## Functional Spec — 핵심 (반드시 만든다)

### 정산단위 엔진

- 정산단위 엔티티: `type`(TRIP/RECURRING/CAMPAIGN), `allowedAccountCodes[]`, `limitKRW`/`limitPeriod`, `approvalLine`, `descriptionTemplate`, `matchKeywords`, `usage`.
- 관리자 콘솔에서 정산단위 작성 폼 (RECURRING/CAMPAIGN) 및 목록/비활성화.
- 모바일 "출장모드 시작"이 동일 API로 TRIP 정산단위 생성.
- 영수증 리뷰 화면에서 활성 정산단위 목록을 후보로 제시, 자동추천은 하이라이트만.
...
- 정산단위 지정 건: 정산단위의 비목·전결라인·적요 템플릿 사용, `usage` 차감.
- 정산단위 미지정 건: 기존 P4 자동분류 + `approval-rules.json` fallback.
- 상신 시 정산단위 한도 초과해도 차단하지 않음 (경고만).
...
| 0:15 | 관리자 콘솔에서 "AI구독료" 정산단위 배포 (신규 장면) |
| 0:30 | 모바일 "출장모드 시작" → 도쿄 TRIP 정산단위 자동 생성, Cap 표시 |
| 0:50 | 영수증 촬영 → 크롭 → OCR → 정산단위 선택(자동추천 하이라이트) → 잔여한도 갱신 |
| 1:30 | 편의점 영수증 촬영 → 적격증빙 경고 시연 → 그대로 진행 |
| 1:50 | 정산하기 → 카드내역 거래 매칭 (기존과 동일) |
| 2:20 | 전표 자동생성 (정산단위의 비목·전결라인 반영) → 상신 |
| 2:40 | 관리자 콘솔 전환 → 전표 접수 확인 |
| 2:55 | AI구독 결제 → 배포된 정산단위 자동 인식 → 잔여한도 차감 시연 |
...
5. **절대 안 버리는 것**: 관리자 정산단위 배포 → 촬영→크롭(원본분리)→OCR→정산단위/비목 선택→거래매칭→전표 자동완성→상신→관리자 화면
...
| 작업 | 내용 | 걸리는 기존 영역 |
|---|---|---|
| 정산단위 API + 관리자 폼 | `server/routes/presets.js`, 관리자 콘솔 정산단위 탭 | P1 연장 |
| 리뷰 화면 정산단위/비목 선택 UI | 모바일 리뷰 화면에 정산단위 칩·비목 칩 추가 | P2 연장 |
| 정산단위 매칭 제안 로직 | `suggestedPresetId` 계산 (matchKeywords/기간 기준) | P4 연장 |
