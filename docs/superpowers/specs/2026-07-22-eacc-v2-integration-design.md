...
> 2026-07-23 운영 보정: 실제 영수증 업로드는 `ocrMode:"real"` 성공 건만 저장한다. API/OCR 실패 시 데모키·SKD 내장 데이터로 전환하지 않는다. 기존 데모는 별도 `/api/receipts/demo`의 명시적 선택으로만 `ocrMode:"demo"` 새 행을 만든다. 상세 계약은 `sot/05_api.md`가 우선한다.
