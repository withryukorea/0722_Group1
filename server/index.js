...
const { db, reset, replaceState, ensureSeedReceipts } = require("./store");
...
app.use(createPersistenceMiddleware({ db, replaceState, persistence }));
...
// 비밀값을 노출하지 않는 영구저장 상태 확인
app.get("/api/persistence-status", (req, res) => res.json(persistence.status()));
...
async function start() {
  ...
  return app.listen(PORT, () => {
    ...
    console.log(`  ├ 저장 모드   : ${persistence.status().mode}`);
