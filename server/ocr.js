...
const OCR_PROMPT = `이 이미지는 영수증입니다. 이미지 전체를 확대해서 줄마다 읽고, 실제로 보이는 글자만 추출하세요. 추측하거나 예시 값을 만들지 마세요.
merchant는 영수증 최상단의 상호·가맹점명(주소나 품목명 아님)을 우선 읽으세요. amount는 공급가액·부가세·단가가 아니라 결제/승인/합계의 최종 금액을 선택하세요.
상호 일부만 선명하면 보이는 부분을 그대로 쓰고, 전혀 판독할 수 없을 때만 null로 두세요. 날짜나 승인번호가 안 보이면 해당 필드만 null로 두되 다른 필드는 계속 추출하세요.
...
function textValue(value) {
  ...
  if (!text || /^(null|undefined|none|n\/?a|없음|미상)$/i.test(text)) return null;
  return text;
}

function normalizeOcr(raw) {
  if (!raw || typeof raw !== "object") return null;
  const merchant = textValue(raw.merchant);
  ...
  return {
    ...
    amount,
    currency,
    paidAt: textValue(raw.paidAt),
    serviceDate: textValue(raw.serviceDate),
    vat,
    invoiceNo: textValue(raw.invoiceNo),
    approvalNo: textValue(raw.approvalNo),
